class HubMsgAdmin {
    ...
    initSidebarMenu() {
        ...
        if (!menuEl) return;
        if (typeof window.MetisMenu === 'function') {
            try {
                if (this.metisMenuInstance && typeof this.metisMenuInstance.dispose === 'function') {
                    this.metisMenuInstance.dispose();
                }
                this.metisMenuInstance = new window.MetisMenu(menuEl);
            } catch (error) {
                console.warn('MetisMenu init failed:', error);
            }
        }
    }
    ...
    switchTab(tabId) {
        ...
        // Update nav state
        document.querySelectorAll('.nav-item.has-treeview').forEach(item => {
        ...
        let activeLink = null;
        document.querySelectorAll('.nav-link[data-target], .icon-bar-item[data-target]').forEach(el => {
            el.classList.remove('active');
        ...
        if (activeLink) {
            const treeItem = activeLink.closest('.nav-treeview')
                ? activeLink.closest('.nav-treeview').closest('.nav-item.has-treeview')
                : activeLink.closest('.nav-item.has-treeview');
            if (treeItem) {
                treeItem.classList.add('menu-open');
                treeItem.classList.add('mm-active');
                const parentLink = treeItem.querySelector(':scope > .nav-link');
                if (parentLink) parentLink.classList.add('active');
                const submenu = treeItem.querySelector(':scope > .nav-treeview');
    ...
    renderSessions(sessions) {
        ...
        if (sessions.length === 0) {
            this.clientList.innerHTML = `
                <div class="col-12 text-center py-5">
                    <i class="fas fa-qrcode text-muted fa-4x mb-3"></i>
                    <p class="text-muted" data-i18n="add_device.qr_waiting">${this.t('add_device.qr_waiting')}</p>
                </div>`;
            return;
        }
        ...
        sessions.forEach(s => {
            ...
            const badgeClass = isReady ? 'badge-success' : 'badge-warning';
            const statusLabel = isReady ? this.t('common.connected') : (s.qr ? this.t('common.qr_pending') : this.t('common.initializing'));

            div.innerHTML = `
                <div class="card h-100 theme-session-card" style="overflow: hidden;">
                    <div class="card-header py-3 px-3 d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center">
                            <div class="rounded-circle p-2 mr-2 theme-chip">
                                <i class="fas fa-mobile-alt text-primary"></i>
                            </div>
                            <h6 class="mb-0 font-weight-bold truncate" style="max-width: 120px;">${s.label}</h6>
                        </div>
                        <span class="badge ${badgeClass} px-2 py-1" style="font-size: 10px;">${statusLabel}</span>
                    </div>
                    <div class="card-body p-4 text-center theme-body" style="min-height: 250px; display: flex; flex-direction: column; justify-content: center; align-items: center;">
                        ${isReady
                    ? `<div>
                                 <i class="fas fa-check-circle text-success fa-4x mb-3"></i>
                                 <h5 class="font-weight-bold text-success">${this.t('common.active')}</h5>
                                 <p class="text-xs text-muted">${this.t('common.security_notice')}</p>
                               </div>`
                    : (s.qr
                        ? `<div class="p-3 rounded border mb-3 theme-qr-wrap">
                                     <img src="${s.qr}" style="width: 180px; height: 180px; display: block;">
                                   </div>
                                   <p class="text-xs text-muted mb-0"><i class="fas fa-camera mr-1"></i> ${this.t('add_device.step4')}</p>`
                        : `<div class="py-4 text-center">
                                     <div class="spinner-border text-primary mb-3" role="status"></div>
                                     <p class="text-sm text-muted">${this.t('common.initializing')}</p>
                                   </div>`)}
                    </div>
                    <div class="card-footer p-3">
                        <div class="d-flex align-items-center" style="gap:8px;">
                            <button class="btn btn-outline-secondary btn-sm rounded-pill font-weight-bold flex-fill" onclick="refQ('${s.id}')">
                                <i class="fas fa-sync-alt mr-1"></i> ${this.t('common.refresh')}
                            </button>
                            <button class="btn btn-outline-danger btn-sm rounded-pill font-weight-bold flex-fill" onclick="delS('${s.id}')">
    ...
    renderDeviceTable() {
        ...
        const flags = {
            ...
            'ae': '🇦🇪', 'in': '🇮🇳', 'br': '🇧🇷', 'ua': '🇺🇦', 'ro': '🇷🇴', 'be': '🇧🇪', 'hu': '🇭🇺'
        };

        this.deviceTableBody.innerHTML = '';
        if (pageItems.length === 0) {
            this.deviceTableBody.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-4">Uyğun cihaz tapılmadı</td></tr>';
        } else {
            pageItems.forEach(d => {
                const tr = document.createElement('tr');
                const flag = flags[d.iso] || '🌍';
                const countryLabel = this.getCountryLabel(d.iso);
                ...
                const adminActions = this.config && this.config.isAdminUser ? `
                    <button class="btn btn-sm btn-link device-delete-btn" onclick="delD('${d.id}')" title="Cihazı sil" aria-label="Cihazı sil">
                        <i class="fas fa-trash-alt"></i>
                    </button>
                ` : '';

                tr.innerHTML = `
                    <td class="col-id"><span class="device-id-text">${d.id ? `#${d.id.slice(-6)}` : '-'}</span></td>
                    <td>
                        <div class="d-flex align-items-center">
                            <div class="mr-2">
                            ...
                            <div>
                                <div class="device-row-title">${d.label}</div>
                                <div class="device-row-phone">+${d.phone}</div>
                            </div>
                        </div>
                    </td>
                    <td>
                        <span class="device-country-badge">
                            <span class="mr-1">${flag}</span> ${countryLabel}
                        </span>
                    </td>
                    <td>
                        <span class="device-platform-main">${platformIcon} <span>${d.platform}</span></span>
                        <span class="device-platform-os">${osIconHtml}<span>${osLabel}</span></span>
                    </td>
                    <td><span class="device-owner-badge">${d.owner}</span></td>
                    <td><span class="device-ready-time"><i class="far fa-clock mr-1"></i>${readyAt}</span></td>
                    <td>
    ...
    updateQueueTable(queue) {
        ...
        if (filteredQueue.length === 0) {
            this.queueTableBody.innerHTML = `<tr><td colspan="6" class="py-4 text-center text-muted">${searchTerm ? 'Uyğun nəticə tapılmadı.' : this.t('bulk.no_messages')}</td></tr>`;
            return;
        }
    ...
    async updateDealerTable() {
        ...
        try {
            ...
            d2.dealers.forEach(dl => {
                ...
                tr.innerHTML = `
                    <td class="font-weight-bold">${dl.username}</td>
